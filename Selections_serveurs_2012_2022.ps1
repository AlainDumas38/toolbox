...
# Obtenez la liste des serveurs Windows 2012, 2016, 2019 et 2022 dans le domaine
$servers = Get-ADComputer -Filter {((OperatingSystem -like "Windows Server 2012*") -or
                                    (OperatingSystem -like "Windows Server 2016*") -or
                                    (OperatingSystem -like "Windows Server 2019*") -or
                                    (OperatingSystem -like "Windows Server 2022*")) -and
                                   (Enabled -eq $true)} -Server $domainName | Select-Object -ExpandProperty Name
